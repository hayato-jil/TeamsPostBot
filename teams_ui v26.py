...
def _wait_message_posted(page: Page, *, text_hint: Optional[str], file_hint: Optional[str], timeout_ms: int) -> bool:
    ...
    while waited < timeout_ms:
        # 1) 配信/既読アイコンの増加
        # （この関数の呼び出し側で base を取ってから呼ぶ設計の方が汎用だが、
        #  ここでは単独でも使えるよう最低限の確認だけ残す）
        # → base 連携は呼び出し側で実施
        # 2) 入力欄が空＝送信でクリアされた
        if _msgbox_is_empty(page):
            return True
        # 3) バブルに本文 or ファイル名の断片が見える
        reg = _latest_message_region(page)
        if reg:
            try:
                inner = (reg.inner_text(timeout=800) or "")
                if text_hint and (text_hint[:8] in inner or text_hint.strip() in inner):
                    return True
                if file_hint and (file_hint[:6] in inner):
                    return True
            except Exception:
                pass
...
def _send_and_confirm(page: Page, *, text_for_hint: Optional[str], file_for_hint: Optional[str]) -> bool:
    ...
    for attempt in range(SEND_RETRIES + 1):
        ...
        # 配信アイコン増加 or 入力欄クリア or バブル検出 のいずれかで成功扱い
        if _wait_delivery_increase(page, base, timeout_ms=DELIVERY_WAIT_MS):
            return True
        if _wait_message_posted(page, text_hint=text_for_hint, file_hint=file_for_hint, timeout_ms=DELIVERY_WAIT_MS):
            return True
        # リトライ前に少し待つ
...
def _try_direct_file_input(page: Page, file_path: str) -> bool:
    ...
    for i in range(min(n, 5)):
        try:
            if file_path != "__DONT_SET__":  # 探索専用のダミー値
    ...
    for idx, (lbl, el) in enumerate(candidates, start=1):
        ...
        if menu is not None:
            ...
            try:
                ...
                if fin.count() > 0:
                    debug(f"attach: found file input(s)= {fin.count()}"); return None
            except Exception: pass
    return None
