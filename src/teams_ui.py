...
# === 置換モーダル対応の待機時間（.envで上書き可） ===
REPLACE_MODAL_WAIT_MS = int(os.environ.get("TPL_REPLACE_MODAL_WAIT_MS", "5000"))
BETWEEN_FILE_ATTACH_MS = int(os.environ.get("TPL_BETWEEN_FILE_ATTACH_MS", "800"))
# =======================================================

def _maybe_handle_replace_modal(page: Page) -> bool:
    """
    「このファイルは既に存在します」ダイアログが出ていれば「置換」をクリックする。
    見つからなければ何もしない。
    """
    try:
        dialog = page.get_by_role("dialog").filter(
            has_text=re.compile(r"このファイルは既に存在します|already exists", re.I)
        )
        dialog.wait_for(timeout=REPLACE_MODAL_WAIT_MS)
    except Exception:
        return False  # ダイアログ自体が出ていない

    try:
        # 置換ボタン（日本語/英語どちらでも）
        replace_btn = dialog.get_by_role("button", name=re.compile(r"置換|Replace", re.I))
        replace_btn.click(timeout=10_000)
        page.wait_for_timeout(350)  # 余韻
        return True
    except Exception:
        return False
...
def open_existing_chat_and_send_message_with_files(*, chat_name: str, message: str, file_paths: list[str]) -> None:
    ...
    各添付の直後に「既に存在します」モーダルが出たら自動で「置換」押下。
    ...
    try:
        ...
        for fp in file_paths:
            ...
            if ok:
                ...
                # 次の操作に移る前の余裕
                page.wait_for_timeout(BETWEEN_FILE_ATTACH_MS)
            else:
                if ATTACH_FAIL_BEHAVIOR == "abort":
