                ...
                if header:
                    ...
                    for i in range(cnt):
                        ...
                        if chat_name in txt:
                            ...
                            return True
                # 2) フォールバック：虫眼鏡行/招待行を除外し、アバターを含む行を優先
...
def create_group_chat_and_send_message(*, admin_email: str, target_email: str,
    ...
    try:
        ...
        _add_recipient(page, to_box, os.environ.get("TARGET_EMAIL", ""))  # 呼び出し側で上書き済み想定

        _set_chat_name_if_available(page, os.environ.get("CHAT_NAME", ""))

        msg_box = _find_message_box(page)
        _type_multiline(page, msg_box, os.environ.get("TPL_MESSAGE", ""))
...
def open_existing_chat_and_send_message_with_file(*, chat_name: str, message: str, file_path: str) -> None:
    """
    既存チャットを検索サジェストから開き、本文を入力し、PDFを1つ添付して送信。
    OSファイルダイアログは Playwright の file_chooser で処理する。
    """
    pw, browser, page = _launch()
    try:
        _ensure_ready(page)
        ...
        # コンポーズ欄にファイルチップが載るのを待つ
        file_name = os.path.basename(file_path)
        attached = False
        for _ in range(40):  # 最大 ~8秒
            try:
            ...
            page.wait_for_timeout(200)
        if not attached:
            print("警告: 添付ファイルのチップを検出できませんでした（送信は継続します）。")
        ...
        if _wait_delivery_increase(page, base_icons, timeout_ms=DELIVERY_WAIT_MS):
            print(f"送信完了：本文＋添付『{file_name}』を投稿しました。（配信確認OK）")
        else:
            print(f"送信は試行しましたが、配信確認が取れませんでした。（ファイル: {file_name}）")
