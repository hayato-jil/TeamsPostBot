...
def _chip_exists(page: Page, address: str) -> bool:
    """
    既に追加済みのピル内テキストに address が含まれるか簡易チェック
    """
    loc = _query_any(page, [
        "[data-tid='people-picker-selected']",
        "[data-tid='people-picker-selectedItem']",
...
# ---- 候補リストを“最新の一つ”に絞って取得し、確実クリック ----
def _latest_listbox(page: Page) -> Optional[Locator]:
    # 画面に複数 listbox が残ることがあるので、最後の可視 listbox を取る
    boxes = page.locator("[role='listbox']")
    try:
        n = boxes.count()
        if n == 0:
            # people-picker系やDropdown系のフォールバック
...
def _click_matching_option(page: Page, address: str) -> bool:
    """
    最新のリストボックスから、可視の option を探してクリック。
    - メール一致を最優先
    - 見つからなければ最初の可視 option をスクロール→ホバー→中央クリック（必要ならforce）
    """
...
def _light_clear_typing_area(page: Page):
    """
    入力欄だけを軽く空にする（ピルは消さない）。
    スペース→Backspace でキャレット位置だけリセット。
    """
    try:
        page.keyboard.type(" ")
...
def _add_recipient(page: Page, to_box: Locator, address: str):
    """
    宛先にメールを入力し、候補を“クリックだけ”で確定（Enterは使わない）。
    ※ 既存のピルを消さない。重複時はスキップ。
    """
    if not address:
        return
    ...
    for _ in range(12):
        ...
        if _click_matching_option(page, address):
            clicked = True
            break
