...
def _menu_pick_device_item(page: Page, menu: Locator) -> Optional[Locator]:
    # メニュー項目を列挙して「このデバイス系」をホワイトリスト選択、クラウド系は除外
    ...
    for i in range(min(count, 10)):
        ...
        try:
            if not it.is_visible(): continue
            txt = (it.inner_text(timeout=800) or "").strip()
            labels.append(txt)
        except Exception:
            continue
    if labels:
        debug(f"attach: menu items= {labels}")

    # ホワイトリスト
    device_pats = [
        r"このデバイス", r"デバイスから", r"コンピューター.*から",
        r"Upload from this device", r"From this device", r"From computer",
    ]
    # NGワード（クラウド系）
    cloud_pats = [r"クラウド", r"OneDrive", r"SharePoint", r"Cloud", r"チーム.*サイト"]
    ...
    for i in range(min(count, 10)):
        ...
        is_cloud = any(re.search(p, txt, re.I) for p in cloud_pats)
        is_device = any(re.search(p, txt, re.I) for p in device_pats)
        if is_device and not is_cloud:
            debug(f"attach: pick device item: {txt}")
            return it
...
def _open_attach_menu_and_pick_device(page: Page):
    """ クリップ/＋ を叩き、メニューから「このデバイス系」を厳選。失敗時は file input フォールバック。 """
    root = _activate_composer(page) or _composer_root(page) or page

    _try_direct_file_input(page, "__DONT_SET__")  # 探索だけ
        ...
        if menu is not None:
            ...
            try:
                with page.expect_file_chooser(timeout=15000) as fc_info:
                    try: item.click(timeout=3000)
                    except Exception: item.click(timeout=3000, force=True)
                return fc_info.value
            except Exception:
                continue
        else:
            # メニュー無し → 直で input[type=file]
