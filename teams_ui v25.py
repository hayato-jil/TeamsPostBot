...
# 添付ボタンのヒント（ツールチップやaria-label）
ATTACH_TOOLTIP_HINT = os.environ.get("ATTACH_TOOLTIP_HINT", "").strip()
ATTACH_FALLBACK_HINTS = [
    "ファイルを添付", "添付", "ファイル", "Attach", "Add file", "Attach a file",
]
...
def _activate_composer(page: Page) -> Optional[Locator]:
    ...
    try:
        msg.scroll_into_view_if_needed(timeout=2000)
    except Exception:
        pass
    try:
...
def _collect_toolbar_buttons(page: Page, root: Optional[Locator]) -> List[Tuple[str, Locator]]:
    """
    添付・プラス候補を composer 配下優先で列挙。 (label, locator)
    """
    labels: List[Tuple[str, Locator]] = []
    search_areas = [root] if root else []
    search_areas.append(page)  # 念のため全体も
    ...
    for area in search_areas:
        ...
        # 1) aria-label / title にヒントを含む button
        for h in hints:
            if not h: continue
        ...
        # 2) data-icon-name (paperclip/attach)
    ...
    # 重複除去（bounding boxの座標で簡易重複判定）
    uniq: List[Tuple[str, Locator]] = []
    seen = set()
    for lbl, el in labels:
        try:
            box = el.bounding_box()
            key = (round(box["x"] if box else 0, 0), round(box["y"] if box else 0, 0))
        except Exception:
            key = (0,0)
...
def _click_button_robust(page: Page, el: Locator, tag: str) -> bool:
    ...
    try:
        ...
        debug(f"attach: click {tag} (normal)")
        return True
    except Exception:
        try:
            el.click(timeout=1500, force=True)
            debug(f"attach: click {tag} (force)")
            return True
        except Exception:
            try:
                box = el.bounding_box()
                if box:
                    ...
                    debug(f"attach: click {tag} (center)")
                    return True
            except Exception:
                pass
    return False
...
def _wait_menu_open(page: Page, timeout_ms: int = 5000) -> Optional[Locator]:
    ...
    while waited < timeout_ms:
        ...
        try:
            if panel.count() > 0 and panel.first.is_visible():
                debug("attach: menu opened")
                return panel.first
        except Exception: pass
        # 一部UIはクリック直後に input[type=file] が増える（メニュー無し）
        root = _composer_root(page) or page
        try:
            if root.locator("input[type='file']").count() > 0:
                debug("attach: file input appeared (no menu)")
                return None
        except Exception:
            pass
        page.wait_for_timeout(step); waited += step
...
def _open_attach_menu_and_pick_device(page: Page):
    """ クリップ or ＋ の候補群から叩き、メニュー or file input を待つ。成功時 FileChooser を返す。 """
    root = _activate_composer(page) or _composer_root(page) or page

    # 直指定（すでに input[type=file] がある）を先に試す
    if _try_direct_file_input(page, "__DONT_SET__") and False:
        pass  # 実際のセットは下流で
    ...
    for idx, (lbl, el) in enumerate(candidates, start=1):
        ...
        if not _click_button_robust(page, el, f"{lbl}#{idx}"): 
            continue
        # メニュー or file input を待つ
        menu = _wait_menu_open(page, 3000)
        if menu is not None:
            # メニューが開いた → 「このデバイスからアップロード」を探す
            device_item = page.get_by_role("menuitem", name=re.compile(r"(このデバイス|デバイスから|アップロード|Upload from this device)", re.I))
            try:
...
def _attach_one_file(page: Page, file_path: str) -> bool:
    ...
    # A) 直指定（既に input[type=file] が生えているなら）
    ...
    # B) メニュー or ボタン叩いて file chooser を掴む
    ...
    # C) file chooser が取れなかったが、input[type=file] はある → 直指定で再挑戦
    if _try_direct_file_input(page, file_path):
...
# ------- テンプレ③ Step2：本文＋1ファイル（チャットタブに依存しない） -------
