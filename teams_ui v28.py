...
# 「このデバイスから」を上書き可能（例: DEVICE_ITEM_HINTS="このデバイスからアップロード,このデバイスから,Upload from this device"）
...
def _wait_menu_open(page: Page, timeout_ms: int = 7000) -> Optional[Locator]:
    waited = 0; step = 140
    page.wait_for_timeout(300)  # 最初のアニメーション待ち
    while waited < timeout_ms:
        ...
        # role=menu が無くてもデバイス項目が見えている可能性 → ここで拾ってしまう
...
def _open_attach_menu_and_pick_device(page: Page):
    ...
    # まず直で input[type=file] が作られていないか確認（UI差異対策）
    ...
    for idx, (lbl, el) in enumerate(candidates, start=1):
        ...
        # 枠内クリック → 通常クリック → force の順で試す
        picked = _click_inside_box(page, item) or _click_button_robust(page, item, "device-item")
        if not picked:
            continue

        # キーボード・フォールバック（フォーカスが乗っていれば↓→Enter）
        try:
            page.keyboard.press("ArrowDown"); page.wait_for_timeout(60); page.keyboard.press("Enter")
        except Exception:
            pass

        # ここで OS のファイルダイアログが来るはず
        try:
    ...
    # メニュー経由が無理でも、input[type=file] が存在すればそれを使用
