...
def _wait_search_suggestion_and_click(page: Page, chat_name: str) -> bool:
    ...
    「グループチャット」/チャット候補をクリック。Enterは押さない。
    """
    # サジェストパネル出現待ち
    page.wait_for_timeout(600)  # タイピング完了から短めの待機
    # 検索候補の共通的なコンテナ
    ...
    while elapsed < 7000:
        try:
            if panel.count() > 0 and panel.first.is_visible():
                # 候補項目
                items = panel.locator("[role='option'], li, div[role='menuitem']")
                cnt = items.count()
                for i in range(cnt):
                    it = items.nth(i)
                    try:
