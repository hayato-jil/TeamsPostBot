...
# 添付待ち・リトライ
ATTACH_UPLOAD_TIMEOUT_MS = int(os.environ.get("ATTACH_UPLOAD_TIMEOUT_MS", "60000"))  # 60秒
...
def _find_search_box(page: Page) -> Optional[Locator]:
    ...
    try:
        page.mouse.move(400, 60); page.mouse.click(400, 60)
...
def _wait_upload_ready(page: Page, file_name: str, timeout_ms: int) -> bool:
    ...
    while waited < timeout_ms:
        ...
        try:
            ...
            if chip.count() == 0:
                chip = page.locator(f"text={short}")
...
def _wait_menu_open(page: Page, timeout_ms: int = 5000) -> Optional[Locator]:
    waited = 0; step = 100
    while waited < timeout_ms:
        panel = page.locator("[role='menu'], [data-tid*='menu']").filter(has_text="このデバイス")
        if panel.count() == 0:
...
def _open_attach_menu_and_pick_device(page: Page):
    """ クリップ→メニュー→『このデバイスからアップロード』を開いて FileChooser を返す """
    # 1) クリップ（paperclip）を押す
    attach_btn = (
    ...
    if not attach_btn:
        debug("attach: clip button not found")
        return None
    try:
    ...
    # 3) メニュー内の『このデバイスからアップロード』を探す
    device_item = (
        page.get_by_role("menuitem", name=re.compile(r"(このデバイス|デバイスから|アップロード|Upload from this device)", re.I))
    )
    try:
        if device_item.count() == 0:
            device_item = menu.locator("div[role='menuitem']:has-text('このデバイス'), button:has-text('このデバイス')")
        device_item.first.wait_for(timeout=3_000)
    except Exception:
        debug("attach: device menuitem not found")
        return None

    # 4) FileChooser を掴んでクリック
    try:
        with page.expect_file_chooser(timeout=15_000) as fc_info:
            try:
                device_item.first.click(timeout=3_000)
            except Exception:
                device_item.first.click(timeout=3_000, force=True)
