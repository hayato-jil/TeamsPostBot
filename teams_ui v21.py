...
# 添付待ち・リトライ
ATTACH_UPLOAD_TIMEOUT_MS = int(os.environ.get("ATTACH_UPLOAD_TIMEOUT_MS", "60000"))  # 60秒待つ
ATTACH_RETRIES = int(os.environ.get("ATTACH_RETRIES", "2"))  # 2回まで再試行
...
def _is_ng_suggestion_text(txt: str) -> bool:
    t = txt.replace("\n", " ")
    ng_patterns = [r"Enter.?キー.*結果.*表示", r"結果.*表示", r"ユーザー.*招待", r"Invite.*to.*Teams"]
            ...
            if panel.count() > 0 and panel.first.is_visible():
                ...
                if header:
                    ...
                    for i in range(cnt):
                        ...
                        try:
                            hb = header.bounding_box(); ib = it.bounding_box()
                            if hb and ib and ib["y"] <= hb["y"]: continue
                        except Exception:
                            pass
                        if chat_name in txt:
...
def _composer_root(page: Page) -> Optional[Locator]:
    # コンポーズ領域の親要素を広めに取得（input[type=file] の探索起点）
    candidates = [
...
def _try_direct_file_input(page: Page, file_path: str) -> bool:
    """ composer 直下や近傍の input[type=file] に直接 set_files する最短経路 """
...
def _wait_upload_ready(page: Page, file_name: str, timeout_ms: int) -> bool:
    waited = 0; step = 300
    while waited < timeout_ms:
        ready_chip = False; uploading = False
        try:
...
def _open_attach_menu_and_pick_device(page: Page) -> bool:
    """ クリップ/ファイル/三点リーダー経由で『このデバイスから』を開く """
    # 1) クリップ系ボタン（アクセシブル名が無くても data-icon 等で拾う）
    attach_btn = (
    ...
    # 2) 三点リーダー配下にある場合
    more_btn = _get_by_role_any(page, [("button", r"その他|More|その他のオプション")]) or _query_any(page, [
        "button[aria-label*='その他']", "button[aria-label*='More']",
        "[data-icon-name*='More']", "button[title*='その他']",
    ])
    if more_btn:
        try:
            debug("attach: try more(...) button")
            more_btn.click(timeout=3000)
        except Exception:
            pass

    # 3) メニューから「このデバイスからアップロード」を選択
    device_opt = (
        _get_by_role_any(page, [("menuitem", r"このデバイス|デバイスから|PC|コンピューター|Upload from this device")]) or
        _query_any(page, [
            "div[role='menuitem']:has-text('このデバイス')",
            "div[role='menuitem']:has-text('Upload from this device')",
            "button:has-text('このデバイス')",
        ])
    )
    if not device_opt:
        return False
    try:
        ...
        return fc_info.value  # FileChooser を返す
    except Exception:
        return False
...
def _attach_one_file(page: Page, file_path: str) -> bool:
    ...
    # A) まずは input[type=file] へ直指定（最も安定）
    ...
    # B) 直指定が無ければ、メニュー経由で OS ダイアログを開く
